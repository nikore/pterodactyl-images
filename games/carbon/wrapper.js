...
function initialListener(data) {
  ...
  if (command === "quit") {
    ...
    console.log(
      'Unable to run "' + command + '" due to RCON not being connected yet.',
    );
  }
}
...
var poll = function () {
  ...
  ws.on("close", function () {
    if (!waiting) {
      ...
      if (retry_count > 5) {
        console.log(
          "connection close retry limit; shutting down the process...",
        );
        exited = true;
        process.exit();
      }
      retry_count++;
      waiting = true;
      console.log("Attempeting %d - Retrying connection...", retry_count);
      setTimeout(poll, 5000);
    }
  });
};
